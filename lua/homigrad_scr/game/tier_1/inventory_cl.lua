if SERVER then
    ...
    AddCSLuaFile("jmod/cl_armor.lua")      -- Клиентские файлы JMod
    -- Подключаем на сервере
    include("autorun/jmod_init.lua")       -- Подключаем для работы на сервере
end

if CLIENT then
    -- Подключаем на клиенте
    include("autorun/jmod_init.lua")
...
local function PopulateItems(parent, items, typ, panelParent, entity, enableFunc, clickFunc)
	parent:Clear()
	...
	for k, itemName in pairs(AlphabetizedItemNames) do
		...
		function Butt:DoClick()
			if self.enabled then
				...
				panelParent.positiveClosed = true
				panelParent:Close()
			else
...
local function StandardSelectionMenu(typ, displayString, data, entity, enableFunc, clickFunc, sidePanelFunc)
	...
	local panelParentss = vgui.Create("DFrame")
	panelParentss.positiveClosed = false
	panelParentss.storted = false
	panelParentss:SetSize(900, 500)
	panelParentss:SetVisible(true)
	panelParentss:SetDraggable(true)
	panelParentss:ShowCloseButton(true)
	panelParentss:SetTitle(displayString)

	function panelParentss:Paint()
	...
	panelParentss:Center()

	function panelParentss:OnKeyCodePressed(key)
		if key == KEY_Q or key == KEY_ESCAPE or key == KEY_E then
			self:Close()
		end
	end

	function panelParent:OnClose()
		if not self.positiveClosed then
			surface.PlaySound("snds_jack_gmod/ez_gui/menu_close.wav")
		end
	end

	local W, H, Myself = panelParent:GetWide(), panelParent:GetTall(), LocalPlayer()
	local Categories = {}
	...
	local TabPanel = vgui.Create("DPanel", panelParent)
	...
	for k, cat in pairs(AlphabetizedCategoryNames) do
		...
		function TabBtn:DoClick()
			...
			PopulateItems(ActiveTabPanel, Categories[ActiveTab], typ, panelParent, entity, enableFunc, clickFunc)
		end
...
local function CreateRemoveButton(parent, x, y, armorID)
    ...
    button.Paint = function(self, w, h)
        draw.RoundedBox(4, 0, 0, w, h, Color(200, 0, 0, 150))
    end
    
    button.DoClick = function()
        net.Start("JMod_Inventory")
        net.WriteInt(1, 8)  -- ActionType 1 для снятия брони
...
function createInventoryPanel(paneloffullscreen,lootEnt, items, items_ammo, isPlayerInventory)
	if true  then
		if not IsValid(paneloffullscreen) then return end
		...
		newPanel:SetSize(0.25 * panelParent:GetWide(), 0.98 * panelParent:GetTall())
		newPanel:SetPos(ScrW()*0.75, ScrH()*0.02) 
