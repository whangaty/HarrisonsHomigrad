...
construct.red = {"Innocent",Color(125,125,125),
    models = tdm.models
}
...
construct.RoundRandomDefalut = 6
...
if SERVER then
    ...
    net.Receive("roundType",function(len)
        construct.roundType = net.ReadInt(4)
        playsound = true
    end)
end

function construct.IsMapBig()
...
function construct.StartRound(data)
    ...
    if SERVER then
        local roundType = homicide_setmode:GetInt() == math.random(1,4) or (construct.IsMapBig() and 1) or false
        construct.roundType = math.random(1,4)
        --construct.roundType = roundType or math.random(2,4)
        --soe, standard, gun-free-zone, wild west
        --print(homicide_setmode:GetString(),construct.roundType)
        net.Start("roundType")
        net.WriteInt(construct.roundType,4)
        net.Broadcast()
    end
...
function construct.HUDPaint_Spectate(spec)
    --local name,color = construct.GetTeamName(spec)
...
function construct.HUDPaint_RoundLeft(white2)
    local roundType = construct.roundType or 2
    local lply = LocalPlayer()
    ...
    if startRound > 0 and lply:Alive() then
        if playsound then
            playsound = false
            surface.PlaySound(roundSound[construct.roundType])
        end
        ...
        if lply.roleT then --Traitor
            if construct.roundType == 3 then --gunfree
            ...
            elseif construct.roundType == 4 then --wildwest
        ...
        elseif lply.roleCT then --Innocent with a gun
            draw.DrawText( DescCT[construct.roundType] or "...", "HomigradRoundFont", ScrW() / 2, ScrH() / 1.2, Color( color.r,color.g,color.b,math.Clamp(startRound - 0.5,0,1) * 255 ), TEXT_ALIGN_CENTER )
        else
